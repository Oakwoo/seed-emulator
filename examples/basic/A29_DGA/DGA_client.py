...
def federated_learning(client):
    ...
    # recording into memory first, then write into file to minimize the
    # I/O effects to running time
    time_accuracy_list = []
    start = time.time()
    ...
    while True:
        try:
            ...
            for k in range(1, K+1):
                # delay D epochs means merge at the D+1 epoch
                # the second condition make sure the merge happends
                # in later round when D < K
                if k == (D % K + 1) and t-1-s >= 1:
                    # Time to delayed Gradient Averaging
                    if (t-1-s) > len(DGA_communication_thread_pool):
                        print("Can't find communication thread!")
                        break
                    else:
                        # block until the delayed gradient comes back
                        DGA_communication_thread_pool[t-1-s-1].join()
                    
                else:
                    # just local train

            # after k epochs, accumulated gradient

            # send t-th round accumulated gradient


            # train on own private data
            model, loss, accuracy = run_centralised(model, lr=0.01)
            
            end = time.time()
            time_accuracy_list.append([end-start, accuracy, loss])
...
def run_centralised(model, lr: float, momentum: float = 0.9):
    ...
    # although here model is changed as same as trained_model, but model in 
    # federated_learning function doesn't changed.
    return trained_model, loss, accuracy

# hyperparameter for DGA
k = 3 # epochs for each iteration
D = 5 # delayed parameter
s = int((D-1)/K)
t = 0
