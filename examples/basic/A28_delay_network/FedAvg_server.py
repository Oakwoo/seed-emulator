...
if __name__ == '__main__':
    ...
    # test inital performance
    loss, accuracy = test(model, testloader)
    time_accuracy_list.append([0, accuracy, loss])
    ...
    for i in range(epochs):
        ...
        for k,v in g_conn_pool.items():
            ...
            # CAUTION! should not use below 2 line join() right after start()
            # will make the parallel running failed
            # thread.start()
            # thread.join()
