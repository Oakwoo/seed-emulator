...
GenerateConfig() {
	...
	# TODO
	# Automatic fetch
        eval processor_count=`cat /proc/cpuinfo| grep "processor"| wc -l`
	...
	# TODO
        ## check whether jq has installed
        if ! type jq >/dev/null 2>&1;
        then echo 'ERROR: jq is not installed!'
             exit
        fi
        ...
        do
            ...
            net_dc="$net_dc    name: $network\n"
            net_dc="$net_dc    external: true\n"
        done
        sed -i "s|SKELETON_SUBNETS|${net_dc}|g" ./confs-$party_id/docker-compose.yml

        ## Load components build material
        components=("proxy" "python" "egg" "fateboard" "federation" "meta-service" "mysql" "redis" "roll")
	...
	sed -i "s|FATEBOARD_HOST_PORT|$((8080+1+i))|g" ./confs-$party_id/docker-compose.yml

	## replace party id avoiding proxy image name overwrite
	sed -i "s|PARTY_ID|${party_id}|g" ./confs-$party_id/docker-compose.yml
...
EOF
    ...
    do
        ...
        sed -i "s|SKELETON_SUBNETS|${net_dc}|g" ./serving-$party_id/docker-compose.yml

        ## Load components build material
        components=("serving-server" "serving-proxy" "redis")
        for component in ${components[*]}
        do
            # if component contains '-' will be treated as code, should use " " to emphasize it.
	...
	## replace party id avoiding proxy image name overwrite
	sed -i "s|PARTY_ID|${party_id}|g" ./serving-$party_id/docker-compose.yml
