...
class FlowerClient(NumPyClient):  # pylint: disable=too-many-instance-attributes
    ...
    def fit(self, parameters, config):
        ...
        '''
        # instead of random generate the straggler schedule to decide how many epochs
        # the straggler should trian, directly give a deadline of trian, once time is
        # run up, submit the result so far.
        # train should return the whole epoch is complete or not, drop_client will
        # decide whether submit this result to server side.
        old_weights = get_weights(self.net)
        old_length_trainloader = len(self.trainloader)
        straggler = train(
            self.net,
            self.trainloader,
            self.device,
        ...
        if straggler and self.configs.run_config.fit.drop_client:
            return (
                    old_weights,
                    old_length_trainloader,
                    {"is_straggler": True},
                )
...
def client_fn(context: Context):
    ...
    #####################################
    limited_time = int(configs.run_config.algorithm.limited_time)
    #####################################
