...
def run(dumpfile = None):
    ...
    gpuhost.addBuildCommand("pip3 install torch numpy torchvision")
    gpuhost.importFile(hostpath = os.path.dirname(os.path.realpath(__file__)) + "/FedAvg_server.py", containerpath = "/FedAvg_server.py").importFile(hostpath = os.path.dirname(os.path.realpath(__file__)) + "/data/test_data_server.pt", containerpath = "/data/test_data_server.pt")
    
    gpuhost2 = as152.createHost('gpu2').setGPUAccess(True).joinNetwork('net0')
    gpuhost2.addSoftware('python3').addSoftware('python3-pip')
    gpuhost2.addBuildCommand("pip3 install torch numpy torchvision")
    gpuhost2.importFile(hostpath = os.path.dirname(os.path.realpath(__file__)) + "/FedAvg_client.py", containerpath = "/FedAvg_client.py").importFile(hostpath = os.path.dirname(os.path.realpath(__file__)) + "/data/train_data_client0.pt", containerpath = "/data/train_data_client.pt").importFile(hostpath = os.path.dirname(os.path.realpath(__file__)) + "/data/test_data_client0.pt", containerpath = "/data/test_data_client.pt")

    gpuhost3 = as152.createHost('gpu3').setGPUAccess(True).joinNetwork('net0')
    gpuhost3.addSoftware('python3').addSoftware('python3-pip')
    gpuhost3.addBuildCommand("pip3 install torch numpy torchvision")
    gpuhost3.importFile(hostpath = os.path.dirname(os.path.realpath(__file__)) + "/FedAvg_client.py", containerpath = "/FedAvg_client.py").importFile(hostpath = os.path.dirname(os.path.realpath(__file__)) + "/data/train_data_client1.pt", containerpath = "/data/train_data_client.pt").importFile(hostpath = os.path.dirname(os.path.realpath(__file__)) + "/data/test_data_client1.pt", containerpath = "/data/test_data_client.pt")
